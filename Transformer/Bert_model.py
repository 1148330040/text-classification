...
checkpoint_path = '../Transformer/models/bert-{}-{}-model'.format(str(time_month), str(time_day))
log_file = '../Transformer/models/bert-{}-{}-logs/'.format(str(time_month), str(time_day))
...
def fit_model(train, test, valid):
    ...
    model.fit_generator(
        ...
        epochs=10,
        callback=[TensorBoard(log_dir=log_file)]
    )

    tf.summary.create_file_writer(logdir=log_file)
